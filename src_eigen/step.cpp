...
static double xij, yij, r2ij;
static double errorSolution,dtNew;
static const double safety = 0.9;

static MatrixXd dv(N,2),k1(N,2),k2(N,2),k3(N,2),k4(N,2),k5(N,2),k6(N,2),sol4(N,2),sol5(N,2);




MatrixXd rungeKutta45(const MatrixXd & xy, const VectorXd & G, double & dt){
...
errorSolution = (sol4 - sol5).array().abs().maxCoeff();
...
MatrixXd computeVelocity(const MatrixXd & A_xy, const VectorXd & A_G){
	dv.setZero();
...
void invokeBoundaryCondtions(MatrixXd & A_xy){
