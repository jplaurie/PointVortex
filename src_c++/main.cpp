...
irowvec structure_fileNumber(NBIN,fill::zeros);
...
double momentumangularValue = 0.0;
...
for(int timeStep = 1; timeStep <= totalSteps; timeStep++){
    ...
    if( int(runTime / outputTime) == fileNumber + 1){
       ...
       //output_vel(vortex_vel,runTime,fileNumber);
        ...
        computeMomentum(vortex_xy, vortex_G, momentumxValue, momentumyValue, momentumangularValue);
        ...
        cout << "file = " << fileNumber << " time = " << runTime << " dt = " << dt << " Ham = " << hamiltonianValue << " MomX = " << momentumxValue << " MomY = " << momentumyValue << " MomAngular = " << momentumangularValue << endl;
    }
}
