...
static double xij, yij,rij,csh,tnh, denom, cir_i, cir_j;

void computeMomentum(mat A_xy, rowvec A_G, double & momentumxValue, double & momentumyValue, double & momentumangularValue){
	momentumxValue = 0.0;
	momentumyValue = 0.0;
...
void computeHamiltonian(mat A_xy, rowvec A_G, double & hamiltonianValue){
...
if(FLAG_BOUNDARY == "infinite"){
	...
	for(int i = 0 ; i < N; i++){
		...
		hamiltonianValue += (0.25/pi) * A_G(i) * A_G(i) * log(cir_i);
	...
	for(int i = 0 ; i < N; i++){
		...
		for(int j = i+1 ; j < N; j++){
			...
			denom = pow(xij,2.0) + pow(yij,2.0);
			cir_i = 1.0 - (  (pow(A_xy(i,0),2.0)  + pow(A_xy(i,1),2.0)  )  /  pow(R,2.0)  );
			cir_j = 1.0 - (  (pow(A_xy(j,0),2.0)  + pow(A_xy(j,1),2.0)  )  /  pow(R,2.0)  );		
			
			hamiltonianValue += (0.25/pi) * A_G(i) * A_G(j) * log( 1.0 +   (pow(R,2.0) * cir_i * cir_j / denom) );
