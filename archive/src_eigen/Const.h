...
//============== Model Parameters ==============
const int Np = 50;      //Number of positive vortices
...
const int totalSteps = 10000;//1.e9;            //Total number of time steps
...
const bool FLAG_DIAGNOSTICS = true;
...
const std::string FLAG_INITIAL_CONDITION = "currentFrame";
...
const double L = sqrt( Lx * Ly / double(N) ) ;	//compute the mean intervortex distance

const double removalDistanceLower = 0.25*L;		//Remove opposite signed vortices if nearest neighbour is closer than this distance
const double removalDistanceUpper = 400.0*L;		//Remove opposite signed vortices if nearest neighbour is futher than this distance
