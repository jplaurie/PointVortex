...
std::vector<std::pair<double, double>> computeVelocities(const VortexSystem& vortices, SimParams params) {
    ...
    for (int i = 0; i < params.N; ++i) {
        ...
        for (int j = 0; j < params.N; ++j) {
            ...
            double dx = vortices.x[i] - vortices.x[j];
            double dy = vortices.y[i] - vortices.y[j];
            double r2 = dx * dx + dy * dy + params.coreSize;

            double coeff = vortices.circ[j] / (2.0 * std::numbers::pi * r2);
            vx += coeff * (-dy);
            vy += coeff * dx;
        }
        velocity[i] = {vx, vy};
...
bool rkf45Step(VortexSystem& vortices, double& dt, double tol = 1e-6){
   ...
   // auto computeStage = [&](const VortexSystem& state) {
  ...
  VortexSystem yk2 = y0;
    ...
    for (int i = 0; i < params.N; ++i) {
        yk2.x[i] += dt * a[1][0] * k1[i].first;
        yk2.y[i] += dt * a[1][0] * k1[i].second;
    }
    ...
    VortexSystem yk3 = y0;
    for (int i = 0; i < N; ++i) {
        yk3.x[i] += dt * (a[2][0] * k1[i].first + a[2][1] * k2[i].first);
        yk3.y[i].y += dt * (a[2][0] * k1[i].second + a[2][1] * k2[i].second);
    }
    auto k3 = computeVelocities(yk3);

    VortexSystem yk4 = y0;
    for (int i = 0; i < params.N; ++i) {
        yk4.x[i] += dt * (a[3][0] * k1[i].first + a[3][1] * k2[i].first + a[3][2]* k3[i].first);
        yk4.y[i] += dt * (a[3][0] * k1[i].second + a[3][1] * k2[i].second + a[3][2] * k3[i].second);
    }
...
*/
    ...
    if (maxError < tol) {
        // Accept step
        for (int i = 0; i < params.N; ++i) {
            vortices.u[i] += (b[0] * k1[i].first + b[1] * k2[i].first+ b[2] * k3[i].first + b[3] * k4[i].first + b[4] * k5[i].first + b[5] * k6[i].first);
            vortices.v[i] += (b[0] * k1[i].second + b[1] * k2[i].second + b[2] * k3[i].second + b[3] * k4[i].second + b[4] * k5[i].second + b[5] * k6[i].second);
            vortices.x[i] += dt * vortices.u[i];
            vortices.y[i] += dt * vortices.v[i];
        }
        ...
        return rkf45Step(vortices, dt, tol);
    }
}
